...
import com.skywin.obd.reader.R;
import com.skywin.obd.reader.activity.ConfigActivity;
import com.skywin.obd.reader.activity.MainActivity;
...
//      ArrayList<ObdCommand> cmds = ConfigActivity.getObdCommands(prefs);
  ...
  private void startObdConnection() throws IOException {
    ...
    try {
      ...
      if(prefs.getBoolean(ConfigActivity.ENABLE_BT_TEST_KEY, false)){
    	  devuuid = UUID.fromString("fa87c0d0-afac-11de-8a39-0800200c9a66");
      }
      sock = dev.createRfcommSocketToServiceRecord(devuuid);
      sock.connect();
    } catch (Exception e1) {
  ...
  protected void executeQueue() throws InterruptedException{
    ...
    while (!Thread.currentThread().isInterrupted()) {
      ...
      if (job != null) {
        ...
        ObdCommand cmd = job2.getCommand();
        if(cmd.cmd!=null&&cmd.getResult()!=null){
        	resultMap.put(cmd.cmd, cmd.getResult());
        }
